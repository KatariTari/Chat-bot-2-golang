...
func (telegramBot *TelegramBot) handleAdminCommand(userID int64), {
	...
	if _, isAdmin := telegramBot.Admins[userID]; isAdmin {
		...
		user := tgbotapi.NewUser(userID)
		msg := tgbotapi.NewMessage(user.ID, "Введите пароль администратора:")
		telegramBot.API.Send(msg)
		...
		adminID, err := authenticateAdmin(userID, password, telegramBot.DB)
		if err != nil {
			msg := tgbotapi.NewMessage(user.ID, "Неверный пароль администратора.")
			telegramBot.API.Send(msg)
			return
		}
...
func authenticateAdmin(userID int64, password string, db *sql.DB) (int64, error) {
	// Запрос к базе данных для проверки пароля администратора.
	// Вместо этого запроса вы можете использовать свой собственный код для проверки пароля.
	// Вот пример запроса:
	var adminID int64
	err := db.QueryRow("SELECT id FROM admins WHERE telegram_id = ? AND password = ?", userID, password).Scan(&adminID)
	if err != nil {
		return 0, err
	}
	return adminID, nil
